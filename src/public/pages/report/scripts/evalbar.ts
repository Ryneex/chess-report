...
const whiteEvalText = document.querySelector(
    "#white-eval-text",
) as SVGTextElement;
const blackEvalText = document.querySelector(
    "#black-eval-text",
) as SVGTextElement;
...
function setEvalTextVisible(visiblePlayer: "white" | "black") {
    if (visiblePlayer === "white") {
        whiteEvalText.setAttribute(
            "visibility",
            boardFlipped ? "hidden" : "visible",
        );
        blackEvalText.setAttribute(
            "visibility",
            boardFlipped ? "visible" : "hidden",
        );
...
async function drawEvaluationBar(
    evaluation: Evaluation,
    boardFlipped: boolean,
    movedPlayer: "white" | "black",
) {
    const blackHeight = Math.max(
        Math.min(totalHeight / 2 - evaluation.value / 3, totalHeight),
        0,
    );
    ...
    if (evaluation.type === "cp") {
        ...
        whiteRect.setAttribute(
            "y",
            boardFlipped ? whiteHeight.toString() : blackHeight.toString(),
        );
        whiteRect.setAttribute(
            "height",
            boardFlipped ? blackHeight.toString() : whiteHeight.toString(),
        );
