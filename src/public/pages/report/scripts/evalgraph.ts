...
const evaluationGraphCtx = (
    $("#evaluation-graph").get(0)! as HTMLCanvasElement
).getContext("2d")!;
...
async function drawEvaluationGraph() {
    ...
    let graphCanvas = document.getElementById(
        "evaluation-graph",
    ) as HTMLCanvasElement;
    graphCanvas.width = desiredGraphWidth;
    graphCanvas.height = graphHeight;

    topLines = positions.map((position) =>
        position?.topLines?.find((line) => line.id == 1),
    );
    ...
    for (let i = 0; i < topLines.length; i++) {
        ...
        let classificationColour = classification
        ...
        if (evaluation?.type === "mate") {
            ...
            if (evaluation.value === 0) {
                evaluationGraphCtx.fillStyle =
                    moveColour === "white" ? "#ffffff" : "#000000";
            } else {
                if (i === currentMoveIndex && i === hoverIndex) {
                    ...
                    evaluationGraphCtx.fillStyle =
                        evaluation.value >= 0 ? "#bbbbbb" : "#555555";
                } else {
                    evaluationGraphCtx.fillStyle =
                        evaluation.value >= 0 ? "#ffffff" : "#000000";
                }
            }
            evaluationGraphCtx.fillRect(
                cumulativeWidth - currentBarWidth,
        ...
        if (i === currentMoveIndex && i === hoverIndex) {
            evaluationGraphCtx.fillStyle = classification
                ? getSemiTransparentColor(classificationColour, 0.8)
            ...
            evaluationGraphCtx.fillStyle = classification
                ? getSemiTransparentColor(classificationColour, 0.5)
                : getSemiTransparentColor("#000000", 0.2);
            evaluationGraphCtx.fillRect(
                cumulativeWidth - currentBarWidth,
                0,
    ...
    for (let i = 0; i < topLines.length; i++) {
        ...
        if (i === hoverIndex) {
            ...
            if (classification && classificationIcons[classification]) {
                ...
                iconY =
                    mouseY > graphHeight - iconSize / 2
                        ? graphHeight - iconSize
                        : iconY;

                if (icon) {
                    const canvasWidth = evaluationGraphCtx.canvas.width;
                    const canvasHeight = evaluationGraphCtx.canvas.height;
                    let speechBubble: any = new (window as any).SpeechBubble(
                        evaluationGraphCtx,
                    );
                    speechBubble.text = "  ";
                    if (hoverIndex % 2 === 0) {
                    ...
                    const bubbleWidth =
                        Math.max(
                            blackPlayer.username.length,
                            whitePlayer.username.length,
                        ) * 12;
                    ...
                    let bubbleGapY = 5;
                    let totalHeight =
                        mouseY + bubbleHeight + bubbleGapY + bubblePadding * 2;
                    let totalWidth =
                        mouseX + bubbleWidth + bubbleGapX + bubblePadding * 2;
                    ...
                    speechBubble.tailStyle = (
                        window as any
                    ).SpeechBubble.TAIL_STRAIGHT;
...
function getMovedPieceByPosition(san: string): string | null {
    ...
    return pieceColour === "white"
        ? prospectivePiece.toUpperCase()
        : prospectivePiece.toLowerCase();
}
